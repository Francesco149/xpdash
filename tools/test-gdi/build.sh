#!/usr/bin/env bash
# build.sh — Cross-compile test-gdi.exe for Windows XP (i686, subsystem 5.1).
set -euo pipefail
...
mkdir -p bin
echo "[*] Compiling test-gdi.exe with $CC..."
"$CC" -O2 -s -Wall -Wextra -D_WIN32_WINNT=0x0501 \
    ...
    src/main.c -o bin/test-gdi.exe -lgdi32 -luser32 -lwinmm

bad="$("$OBJDUMP" -p bin/test-gdi.exe | awk '/DLL Name/{print $3}' \
...
echo "[+] Successfully built bin/test-gdi.exe ($(stat -c%s bin/test-gdi.exe) bytes)"
