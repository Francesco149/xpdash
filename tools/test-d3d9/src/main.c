/*
 * tools/test-d3d9/src/main.c — Direct3D 9 Graphics & Display Configuration Test
 *
 * Tests Windows XP Direct3D 9 hardware 3D rendering, backbuffer presentation,
 * fullscreen exclusive mode, depth buffering (Z-Buffer), 16-bit vs 32-bit color,
 * hook injection capture (Path A), and GDI BitBlt capture fallback (Path B).
 *
...
#define WINDOW_CLASS_NAME "XpDash_TestD3D9"
...
static void mat_look_at_lh(D3DMATRIX *out, float eye_z) {
    mat_identity(out);
    out->_43 = eye_z; // Camera looking along +Z from -eye_z
}
...
static int init_d3d9(int w, int h, int bpp, int fullscreen) {
    cleanup_d3d9();
    ...
    printf("[test-d3d9] Initialized D3D9: %dx%d @ %d-bit (%s)\n",
...
static void render_hud_bars(int width) {
    ...
    g_pDevice->lpVtbl->SetFVF(g_pDevice, D3DFVF_HUD);
...
static void render_frame(float dt) {
    ...
    if (hr == D3DERR_DEVICELOST) {
        ...
        printf("[test-d3d9] Device needs reset, resetting...\n");
    ...
    if (SUCCEEDED(g_pDevice->lpVtbl->BeginScene(g_pDevice))) {
        g_pDevice->lpVtbl->SetFVF(g_pDevice, D3DFVF_CUBE);
        ...
        case WM_KEYDOWN:
            switch (wParam) {
                ...
                case VK_RETURN:
                    ...
                    init_d3d9(g_target_w, g_target_h, g_target_bpp, g_fullscreen);
                    return 0;
...
int main(int argc, char **argv) {
    ...
    printf("====================================================\n");
    printf(" xpdash Direct3D 9 Test Application\n");
    ...
    g_hwnd = CreateWindowExA(
        0, WINDOW_CLASS_NAME, "xpdash Direct3D 9 Test",
    ...
    if (!init_d3d9(g_target_w, g_target_h, g_target_bpp, g_fullscreen)) {
        printf("[-] init_d3d9 failed!\n");
        cleanup_d3d9();
        return 1;
    }
    ...
    while (running) {
        ...
        if (g_auto_seconds > 0) {
            if ((now - g_start_time) >= (DWORD)(g_auto_seconds * 1000)) {
                printf("[test-d3d9] Auto-exit duration (%ds) elapsed. Exiting cleanly.\n", g_auto_seconds);
                running = 0;
                break;
            }
        }
        ...
        if (now - g_last_fps_time >= 500) {
            ...
            snprintf(title, sizeof(title),
                     "xpdash D3D9 Test - %s (%dx%d @ %dbpp) - %.1f FPS - Frame %lu",
    ...
    timeEndPeriod(1);
    cleanup_d3d9();
    printf("[test-d3d9] Test complete. Total frames rendered: %lu\n", g_total_frames);
    return 0;
}
