#!/usr/bin/env bash
# build.sh — Cross-compile eax-test.exe for Windows XP (i686, subsystem 5.1).
set -euo pipefail
...
mkdir -p bin
echo "[*] Compiling eax-test.exe with $CC..."
"$CC" -O2 -s -Wall -Wextra -D_WIN32_WINNT=0x0501 \
    ...
    src/main.c -o bin/eax-test.exe -lwinmm

# XP-safety: check imported DLLs
bad="$("$OBJDUMP" -p bin/eax-test.exe | awk '/DLL Name/{print $3}' \
...
echo "[+] Successfully built bin/eax-test.exe ($(stat -c%s bin/eax-test.exe) bytes)"
